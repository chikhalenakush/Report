...
Browser automation is used in our work to automate Applet wrapper, i.e the process of uploading data from local system to server of Carelink via browser. The problem lies in not able to run applet as standalone application, because it uses signed certificate with login cookies from browser. We have tried to emulate user behavior in performing steps manually uploading data to server. 
...
With respect to project work in this thesis, All the three main modules are tested with Junit as unit test case Framework. At the very beginning Crawler module i.e downloading CSV file is tested. Within Crawler module submodule Login functionality is first with input from user. Thereafter, start date and end dates are tested to validate correct dates for CSV Download.Test case contains an array of predeinfed values and their result. A quick example to enter date Values would be something like
\begin{lstlisting}
...
\end{lstlisting}
 Within the same module CSV path will also be tested so that the entire module for Crawler (Downloading CSV file) is tested.
 \begin{lstlisting}
...
\end{lstlisting}
 To test Crawler CSV download module, Command used is "java -jar abc.jar -crawler -test". Important point to note is that password is always masked while running from both command line and through IDE, so it does not appear in clear text. Applet Module is applied for testing. 
 
 Second step in Unit Testing is to Test Applet wrapper module which consist of three main parts.
 \begin{enumerate}
\item Login credentials
\item SN Number and Device validation
\item Run automated browser early on 
\end{enumerate}
Login is tested as same by calling the previous methods. Thereafter SN Number and Device are tested withinin module of Applet wrapper. Taking from Section
~\ref{subsec:AppletWrapper} the correct definiftion of SN number and Device is being stated. Using the same definition SN number and Device are checked as shown below in code snippet
...
\end{lstlisting}   
    ...
    To test this module command used is "java -jar abc.jar - applet -test" A code snippet class for a generic Test case is shown below.
\begin{lstlisting}
